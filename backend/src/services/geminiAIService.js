...
class GeminiAIService {
  constructor() {
    ...
    // In-memory cache for health check response (avoids burning 5 RPM free tier quota on every HTTP ping!)
    this._healthCache = null;
    this._healthCachedAt = 0;
    this._healthInFlight = null;
    this.HEALTH_CACHE_TTL_MS = 60 * 1000; // 60 seconds TTL
    this.HEALTH_ERROR_CACHE_TTL_MS = 90 * 1000; // 90s for error status (longer TTL to avoid 429 storm on repeated pings)
    ...
    if (this.aiEnabled && this.apiKey) {
      try {
        ...
        logger.info(`[GeminiAIService] Initialized with model: ${this.modelName}`);
        // Do NOT warm health cache immediately — constructor boot is critical path & 4 retries may sleep 3 minutes overlapping frontend pings
        // First /health or health endpoint hit will trigger lazily with concurrency guard.
      } catch (err) {
        logger.error(`[GeminiAIService] Failed to initialize`, { error: err?.message || String(err) });
        this.aiEnabled = false;
      }
    } else {
      logger.warn(`[GeminiAIService] AI is disabled or API key not configured`, {
        hint: 'Set AI_ENABLED=true and GEMINI_API_KEY in .env',
      });
    }
  }
  ...
  async _generateContent(prompt, options = {}) {
    if (!this.isEnabled()) {
      throw new Error('AI service is not enabled. Please configure AI_ENABLED=true and GEMINI_API_KEY.');
    }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (options.responseMIMEType && /^application\/json$/i.test(options.responseMIMEType)) {
          generationConfig.responseMimeType = 'application/json';
        }

        const effectiveModel = this.genAI.getGenerativeModel({
          model: this.modelName,
          generationConfig,
        ...
        const isRetryable = /429|5\d{2}|Too Many Requests|quota|retry|timeout|ECONNRESET|ETIMEDOUT|network/i.test(msg);
        const retryDelayMs = isRetryable
          ? Math.max(this._extractRetryDelayMs(msg), 1000 * Math.pow(2, attempt))
          : 0;

        logger.warn(`[GeminiAIService] generateContent attempt ${attempt + 1}/${maxRetries + 1} failed`, {
    ...
    logger.error(`[GeminiAIService] generateContent FAILED after all ${maxRetries + 1} attempts`, {
...
RESPONSE FORMAT (JSON only):
{
  ...
  // ============================================================
  // FEATURE 4: TEMPLATE FIELD SUGGESTOR
  // ============================================================
...
Ensure fieldKey is unique, uppercase snake_case, descriptive but not too long. Cover ALL key information needed for "${documentType}".`;
  ...
  // ============================================================
  // FEATURE 5: CHAT BOT (Multi-turn conversation + DMS context)
  // ============================================================
  async chat({ messages, pageContext, dmsUser, language, temperature }) {
    ...
    try {
      ...
      logger.error('[GeminiAIService] chat failed:', err.message);
...
7. LANGUAGE DOUBLE-CHECK: Before you send your final response, verify:
  ...
  // ============================================================
  // FEATURE 6: NATURAL LANGUAGE DOCUMENT SEARCH
  // ============================================================
...
OUTPUT (strict JSON only):`;
  ...
  _safeParseJSON(raw, fallback) {
    ...
    try {
      ...
      try {
        ...
        logger.warn('[GeminiAIService] JSON parse failed, returning fallback', { error: err2?.message || String(err2) });
      }

      logger.warn('[GeminiAIService] Could not parse AI response as JSON, returning fallback');
      return fallback;
    }
  }
  ...
  async healthCheck(options = {}) {
    ...
    // CONCURRENCY GUARD: if real Google probe already running, await same promise — NEVER spawn overlapping parallel 4-retry loops
    ...
    // Fast local-only response (no Google hit) if status was already error AND caller doesn't need deep check
    ...
    this._healthInFlight = (async () => {
      try {
        ...
        // For health check ping use maxRetries=0 — NO EXPONENTIAL RETRY LOOP inside ping (retry causes multi-minute sleep storm with overlapping threads!)
    ...
    try {
      ...
      // Keep in-flight for 100ms as quick-debounce for burst pings
      setTimeout(() => { this._healthInFlight = null; }, 100);
    }
  }

  // ============================================================
  // FEATURE 7: REPHRASE / REWRITE TEXT (Formal, Simple, Concise, Expand, Grammatical)
  // ============================================================
...
`;
  ...
  // ============================================================
  // FEATURE 9: REVIEWER REMARKS GENERATOR (Approve / Revise / Reject)
  // ============================================================
...
`;
  ...
  // ============================================================
  // FEATURE 10: GRAMMAR & SPELLING CHECKER (BM + EN)
  // ============================================================
