...
export default function TenderFollowUpModal({ open, entry, onClose, onSaved }) {
  ...
  const tenderLabel = useMemo(() => {
    const ref = String(entry?.tenderRefNo || '').trim()
  ...
  const refreshAssignees = async () => {
    ...
    try {
      const res = await api.get(`/crm/tender-book/${entryId}/assignees`)
  ...
  const refreshFollowUps = async () => {
    ...
    try {
      const res = await api.get(`/crm/tender-book/${entryId}/follow-ups`)
  ...
  const saveAssignees = async () => {
    ...
    setError('')
    try {
      await api.put(`/crm/tender-book/${entryId}/assignees`, { userIds: assigneeIds })
      onSaved?.()
    } catch (e) {
  ...
  const submitFollowUp = async () => {
    ...
    try {
      ...
      await api.post(`/crm/tender-book/${entryId}/follow-ups`, payload)
      setFollowUpDraft({ followUpAt: '', assignedToId: '', note: '' })
        ...
        <div className="px-6 py-4 border-b border-gray-200 sticky top-0 bg-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-600 mt-1">{tenderLabel}</p>
            </div>
