      ...
      >
        <div className="border-r border-border">
          {hours.map((h) => (
            <div
              key={h}
              className="h-[52px] border-b border-border/50 last:border-b-0 text-[11px] text-ink-muted text-right pr-2.5 pt-2 font-medium"
            >
              {pad2(h)}:00
            </div>
          ))}
            ...
            <div key={d.toISOString() + '_col'} className="relative border-r border-border last:border-r-0">
              ...
              {hours.map((h) => {
                ...
                return (
                  <div
                    key={h}
                    className="h-[52px] border-b border-border/50 last:border-b-0 p-1.5 relative hover:bg-[var(--dms-color-info-soft)]/20 cursor-pointer transition-colors"
                    onClick={(e) => {
                      e.stopPropagation()
