      ...
      <div className="grid grid-cols-[60px_minmax(0,1fr)] flex-1 border border-border rounded-b-2xl overflow-hidden bg-[var(--dms-color-bg-surface)]/80">
        <div className="border-r border-border">
          {hours.map((h) => (
            <div
              key={h}
              className="h-[72px] border-b border-border/50 last:border-b-0 text-[10px] text-ink-muted text-right pr-2 pt-1"
            >
              {pad2(h)}:00
            </div>
          ))}
          ...
          {hours.map((h) => {
            ...
            return (
              <div
                ...
                className="h-[72px] border-b border-border/50 last:border-b-0 p-1.5 hover:bg-[var(--dms-color-info-soft)]/25 cursor-pointer transition-colors"
              >
